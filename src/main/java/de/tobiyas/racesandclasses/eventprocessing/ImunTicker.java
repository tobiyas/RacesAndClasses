...
public class ImunTicker implements Runnable {
	...
	public void init(){
		synchronized(imunMap){
			imunMap.clear();
			imunTicks = (int) Math.ceil(plugin.getConfigManager().getGeneralConfig().getConfig_imunBetweenDamage() / 50);
			if(taskID > 0)
		...
		useInternSystem = plugin.getConfigManager().getGeneralConfig().isConfig_useInternImunSystem();
	}
