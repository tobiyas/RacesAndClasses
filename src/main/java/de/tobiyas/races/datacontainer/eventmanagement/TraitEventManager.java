...
import de.tobiyas.races.datacontainer.eventmanagement.events.EntityDamageByBlockDoubleEvent;
...
import de.tobiyas.races.datacontainer.eventmanagement.events.EntityKnockbackEvent;
		...
		if(event instanceof EntityDamageDoubleEvent){
			...
			if(Eevent.getEntityType() == EntityType.PLAYER){
				...
				if(damage > 0){
					...
					entity.setLastDamageCause(Eevent.generateBukkitEvent());
				}
			}
