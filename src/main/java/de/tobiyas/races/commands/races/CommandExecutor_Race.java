...
public class CommandExecutor_Race extends Observable implements CommandExecutor {
	...
	private void raceList(Player player){
		ArrayList<String> races = RaceManager.getManager().listAllVisibleRaces();
		RaceContainer playerRace = RaceManager.getManager().getRaceOfPlayer(player.getName());
		if(playerRace == RaceManager.getManager().getDefaultContainer())
			races.add(RaceManager.getManager().getDefaultContainer().getName());
