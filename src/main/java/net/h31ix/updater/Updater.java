...
public class Updater
{
    ...
    private String [] noUpdateTag = {"-DEV","-PRE","-SNAPSHOT", "BETA", "ALPHA"}; // If the version number contains one of these, don't update.
    ...
    private boolean versionCheck(String title)
    {
        if(type != UpdateType.NO_VERSION_CHECK)
        {
            ...
            else
            {
                ...
                plugin.getLogger().warning("The author of this plugin (" + plugin.getDescription().getAuthors().get(0) + ") has misconfigured their Auto Update system");
                plugin.getLogger().warning("Files uploaded to BukkitDev should contain the version number, separated from the name by a 'V', such as PluginName V1.0");
